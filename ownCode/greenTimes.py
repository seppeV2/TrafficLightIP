...
def get_green_times(flows, assignment, method, oldGreenTimesDic, ff_tt, g, signal_node_link_connect, costMethod ):
    print('Run Green Times')
    ...
    for sign_node_id in signal_node_link_connect.keys():
        ...
        if method == 'equisaturation':
            greenTimes = equisaturationGreenTimes(intersectionCaps, intersectionLinksFlows, oldGreenTimes, intersectionFf_tt, method, signal_node_link_connect[sign_node_id], costMethod)
        elif method == 'P0':
            greenTimes = P0policyGreenTimes(intersectionCaps, intersectionLinksFlows, oldGreenTimes, intersectionFf_tt, method, signal_node_link_connect[sign_node_id], costMethod)
...
def msa_green_times(caps, flows, initial_greens, ff_tts, method, link_ids, costMethod):
    converged = False
    ...
    while not converged:
        ...
        if method =='equisaturation':
            ...
            equality = [c*get_link_delay(x,c,ff_tt,g_time, costMethod) for c,x,ff_tt,g_time in zip(caps, flows, ff_tts, greens)]
    ...
    print(f'Converged because: {converged_reason}, equality conditions = {equality}')
    print(f'Green times for this intersection = {previous_greens}\n')
...
def equisaturationGreenTimes(caps, flows, initial_greens, ff_tts, method, link_ids, costMethod):
    #now calculate the green times iteratively 
    greens = msa_green_times(caps, flows, initial_greens, ff_tts, method, link_ids, costMethod)
    
    return greens


def P0policyGreenTimes(caps, flows, initial_greens, ff_tts, method, link_ids, costMethod):
...
def get_link_delay(flow, cap , ff_tt, g_time, costMethod):
    if costMethod == 'bpr': 
        return __bpr_green_cost_single(flow, cap, ff_tt , g_time) - __bpr_green_cost_single(0, cap, ff_tt , 1)
    
    elif costMethod == 'WebsterTwoTerm': 
        return __webster_two_term_green_single(flow, cap, ff_tt, g_time) - __webster_two_term_green_single(0, cap, ff_tt, 1)
    
    elif costMethod == 'hybrid':
        return __hybrid_cost_single(flow, cap, ff_tt, g_time) - __hybrid_cost_single(0, cap, ff_tt, 1)
