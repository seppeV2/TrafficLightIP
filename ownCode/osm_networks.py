...
def make_osm_network(place: str, distance_outer: int, distance_inner:int):
    ...
    X,Y, name, place = auto_configured_centroids(place, buffer_dist_close = distance_inner, buffer_dist_extended=distance_outer)
    res = generate_od_xy(30,'Leuven',2000)
    """
    auto_centroid = {'X':[], 'Y':[],'name':[], 'place':[]}
    for x,y,_name, _place in zip(X, Y, name, place):
        if _place != '-':
            auto_centroid['X'].append(x)
            auto_centroid['Y'].append(y)
            auto_centroid['name'].append(_name)
            auto_centroid['place'].append(_place)
...
make_osm_network('Leuven', 2000,2500)