...
import os
import osmnx as ox
from demand.extract_demand import generate_demand
from demand.extract_demand import generate_demand_2d
from pathlib import Path
import math


def STA_initial_setup(place: str, buffer_D:int, buffer_N:int,buffer_transit:int, D_sup:float, time_of_day:int,ext: int, path_parent = ''):
    '''
    ...
    **BE CAUTIOUS ABOUT SOME PARTIALLY OF ILL LOADED FILES FROM FAILED RUNS OF PREVIOUS TIME MAY LIE IN THE FILE PATHS AND
    ...
    R_D = buffer_D
    R_N = buffer_N
    path = os.path.join(Path(__file__).parent,'shapefile_data','Full projected centroids','Full projected centroids.shp')
    ...
    shp_name = place + '_' + str(R_D)+ '_' + str(int(D_sup*10)) if path_parent == '' else place# D_sup*10 is only to avoid the decimal point in file name. NO OTHER SIGNIFICANCE
    
    filepath_shp = os.path.join(Path(__file__).parent,'shapefile_data', str(shp_name)) if path_parent == '' else os.path.join(path_parent, str(shp_name)) # this is the directory where the shape file and other accompanying files will be saved
    os.makedirs(filepath_shp, exist_ok=True)
    zoning_file = os.path.join(str(filepath_shp), shp_name + '.shp')  # the particular .shp file
    m1=0
    ...
    if not os.path.isfile(zoning_file):  # only if such a file doesn't exist already
        ...
        shp.to_file(zoning_file, encoding='UTF-8')
    ...
    filepath_osm = os.path.join(Path(__file__).parent,'network_data', str(network_name))
    m2=0
    ...
    filepath_full_od_matrix = os.path.join(Path(__file__).parent,'od_matrix_data', 'full_matrices', str(od_matrix_name))
    m4 = 0

    if not os.path.isfile(filepath_full_od_matrix):  # if file not already present- for the time of day
        m4 = 1
        dynamic_od_table_csv = os.path.join(Path(__file__).parent,"Verplaatsingen_bestuurder_uur_spmVlaanderenVersie4.2.2_2017.CSV")
    ...
    if ext ==0:
        ...
        reduced_od_matrix_name = place + '_' + str(R_D) + '_' + str(time_of_day) + '_' + ".xlsx" if path_parent == '' else place + ".xlsx"
        path = os.path.join(Path(__file__).parent, 'od_matrix_data')
        os.makedirs(path, exist_ok = True)
        filepath_od_matrix = os.path.join(path, str(reduced_od_matrix_name)) if path_parent == '' else os.path.join(path_parent, str(reduced_od_matrix_name))
        m3 = 0
        ...
        od_matrix_ext_name = place + '_ext' + '_' + str(R_D) + '_' + str(time_of_day) +  '_' + str(int(D_sup*10))+ ".xlsx" if path_parent == '' else place + ".xlsx"
        path = os.path.join(Path(__file__).parent, 'od_matrix_data_ext')
        os.makedirs(path, exist_ok = True)
        filepath_od_matrix_ext = os.path.join(path, str(od_matrix_ext_name)) if path_parent == '' else os.path.join(path_parent, str(od_matrix_ext_name))
        ...
        od_matrix_ext_tr_name = place + '_ext_tr' + '_' + str(R_D) + '_' + str(time_of_day) + '_' + str(int(D_sup*10))+ ".xlsx" if path_parent == '' else place + ".xlsx"
        path = os.path.join(Path(__file__).parent, 'od_matrix_data_ext_tr')
        os.makedirs(path, exist_ok = True)
        filepath_od_matrix_ext_tr = os.path.join(path, str(od_matrix_ext_tr_name)) if path_parent == '' else os.path.join(path_parent, str(od_matrix_ext_tr_name))
        m6 = 0
