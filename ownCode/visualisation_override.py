...
def show_network_own_real(
    ...
    node_source = _node_cds(tmp, highlight_nodes, **node_kwargs)
    data_signal = {'x': [], 'y':[], 'node_id':[]}
    data_od = {'x': [], 'y':[], 'node_id':[]}
    for id, x, y, x_cor, y_cor in zip(node_source.data['node_id'],node_source.data['x'],node_source.data['y'],node_source.data['x_coord'],node_source.data['y_coord']):
        if id in signalized_nodes:
            data_signal['x'].append(x)
            data_signal['y'].append(y)
            data_signal['node_id'].append(id)
        if (x_cor, y_cor) in O_or_D:
            data_od['x'].append(x)
            data_od['y'].append(y)
            data_od['node_id'].append(id)

    source1 = ColumnDataSource(data = data_signal)
    source2 = ColumnDataSource(data = data_od)
    if show_nodes:
        node_renderer = plot.add_glyph(
            node_source,
            glyph=Circle(
...
def get_signal_node_list(g, signalized_nodes):
    ...
    for v in g.nodes:
        if g.nodes[v]['node_id'] in signalized_nodes:
            x_coord.append(g.nodes[v]['x'])
            y_coord.append(g.nodes[v]['y'])
            id_s.append(str(g.nodes[v]['node_id']))
