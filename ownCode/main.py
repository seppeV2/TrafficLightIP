...
def main(network_type, methodCost, methodGreen, greenDistribution, flow):
    ...
    return ([methodCost, methodGreen, firstGreens[8], firstGreens[14], newResult.link_costs[4], newResult.link_costs[5], newResult.link_costs[8], newResult.link_costs[14], newResult.flows[4], newResult.flows[5], newResult.flows[8], newResult.flows[10], newResult.flows[12], newResult.flows[14], newGreens[8], newGreens[14], safety_loop])

import pandas as pd
col = ['Cost','Policy','0-3','1-3','2-3','ig8','ig14', 'fc4','fc5','fc8','fc14','flow4', 'flow5','flow8','flow10','flow12','flow14','fg8','fg14','Loops']
two_merge = pd.DataFrame(columns=col)

net='twoMerge'
cost = 'bpr'
greens = ['20-80','40-60','equal','60-40','80-20']
policies = ['equisaturation','P0']
demands_bpr = [(80,20,20),(80,30,20),(80,30,30),(90,30,20), (100,20,20), (110,20,30),(120,20,20)]
demands_w2t = [(80,20,20),(80,30,30),(100,20,20),(120,20,20),(135,20,20),(145,20,20)]
for pol in policies:
    for (d1,d2,d3) in demands_bpr:
        for g in greens:
            results = main(net, cost, pol, g, [(0,3,d1),(1,3,d2),(2,3,d3)])
            demand_list=[d1,d2,d3]
            results[2:2] = demand_list
            two_merge.loc[len(two_merge.index)] = results

two_merge.to_excel(net+'_'+cost+'.xlsx', index=False)